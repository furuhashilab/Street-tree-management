...
## 1. はじめに
都市における街路樹は、街の景観向上（シンボルツリー、新芽・花・紅葉などで四季を感じるなど）、生活環境の保全（騒音低下、夏の緑陰確保、ヒートアイランド現象の抑制、大気の浄化）、交通安全（歩道と車道の分離、眩しさを遮る）、防災（火事の延焼を防ぐ）などの役割がある。しかし自然界ではなく、道路沿いという環境に植えられた現代の街路樹は、道路標識の視認性確保のためや安全通行のため強剪定されたり、強風・地震・積雪などで倒れ歩行者が人命を奪われたり、交通障害を起こしたりする。また、災害時は避難や救助活動の障害になることもある。日本では住宅開発が進んだ1960年～1980年代に大量に植えられた街路樹が老齢化し、倒木被害などの問題も顕在化しつつある。植えられた街路樹は管理を怠らず日常的な点検・剪定・改善措置の実行などのメンテナンスをしっかりと行い、異常や危険などの問題を回避する必要がある。そのためには、行政だけでなく、住民も日頃から身近な街路樹に対し関心を持ち行政と住民の協働が望まれる。
...
上記問題点を解決するため、本研究では誰でも簡単に利用・編集できるOpenStreetMapに街路樹をマッピングし、街路樹の位置情報、属性を共有し、Mapillaryで街路樹の時系列変化を記録、LocalWikiで樹木（特に問題のある樹木）の生育状態の経時的変化を過去の写真も利用しながら観察記録をとる。街路樹管理支援のため住民が使いやすく継続して利用できる方法の構築を試みた。
...
街路樹や道路全般の障害を自治体に知らせるスマホアプリとLocalWikiを利用した観察記録を比較したとき、観察記録は一過性でないため日頃の手入れなど樹木管理の改善措置をとる時に有効に活用できると思われる。、
...
*観察記録という観点では四季の変化が認められる３か月おきくらいに樹木が観察されることが望ましい。

### 5 結論
本研究では行政と住民の協働が望まれる街路樹管理において、住民サイドからの支援として既存の無料ツールを統合的に運用する方法を提案し、その有効性が確認できた。　課題はまだあるが、行政と住民が話し合いながらより良い街路樹管理ができるようにさらに検証する必要がある。
